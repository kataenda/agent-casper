...
> **Agent-to-agent commerce (the machine economy), working today.** Agent Casper is not just an x402 *consumer* — it *earns* over x402. An independent buyer agent with its own ed25519 identity (`buyer_key.pem`, `00e2d5cd…`) pays the Agent Casper provider (`0088cb6d…`) and the facilitator settles it **on-chain**: a real CEP-18 `transfer_with_authorization` moving the X402 token between two distinct agents — verified on testnet, tx [`eb0e914c…`](https://testnet.cspr.live/transaction/eb0e914cdd902b177d95cd92a345cff3d7cdfbc33bffe8927d456d8c8a1f469e). Autonomous machine-to-machine payment, no human in the loop. Reproduce: `scripts/fund_buyer.py` → `scripts/buyer_pays_agent.py`. This closed, two-sided loop (the agent both pays for its RWA risk feed and gets paid for its AI decisions) is exactly the agent economy described in the Casper Manifest, on top of real DeFi + RWA execution.
...
| **Agent-to-agent x402 settle (independent buyer → provider)** | `eb0e914cdd902b177d95cd92a345cff3d7cdfbc33bffe8927d456d8c8a1f469e` | [View on cspr.live](https://testnet.cspr.live/transaction/eb0e914cdd902b177d95cd92a345cff3d7cdfbc33bffe8927d456d8c8a1f469e) |
